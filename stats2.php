...
$res = mysql_query("SELECT id, username, uploaded, freeleech, real_seedtime_update, real_leechtime_update FROM users WHERE passkey = " . sqlesc($passkey) . " LIMIT 1") or err("Errno: 1");
...
if ($self)
{
	...
	if (!$left && !$self["left"])
	{
		...
		$userd[] = "real_seedtime_update = '" . get_date_time() . "'";
		$snatch[] = "seedtime = seedtime + $timespent";
		
		$timediff = time() - strtotime($user["real_seedtime_update"]);
		
		if ($timediff < 40 * 60)
			$userd[] = "real_seedtime = real_seedtime + $timediff";
		else
			$userd[] = "real_seedtime = real_seedtime + $timespent";
	}
	else
	{
		$userd[] = "real_leechtime_update = '" . get_date_time() . "'";
		$snatch[] = "leechtime = leechtime + $timespent";
		
		$timediff = time() - strtotime($user["real_leechtime_update"]);
		
		if ($timediff < 40 * 60)
			$userd[] = "real_leechtime = real_leechtime + $timediff";
		else
			$userd[] = "real_leechtime = real_leechtime + $timespent";
	}
